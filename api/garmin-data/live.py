"""
POST /api/garmin-data/live
Body: {"token": str}

Lightweight intraday numbers for the dashboard's live panel: steps,
current heart rate (plus today's intraday series), body battery,
stress, calories. Only 4 Garmin calls, so the client can poll this
every few minutes without tripping rate limits — unlike the full
snapshot, which fans out to ~30 calls.

Returns a refreshed `token` like snapshot.py does, so the session
cookie stays current.
...
try:
    from _garmin_lib import INTERNAL_FN_SECRET, resume_client, dump_token, get_today_live
...
class handler(BaseHTTPRequestHandler):
    ...
    def _handle(self) -> None:
        ...
        _send(self, 200, {"token": fresh_token, "data": live})
